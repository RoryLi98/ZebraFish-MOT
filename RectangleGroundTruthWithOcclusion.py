...
# GroundTruth = np.loadtxt('D:\\bishe\\3DZeF20\\3DZeF20\\train\\ZebraFish-04\\gt\\gt.txt', delimiter=',' , usecols=[0,1,14,15,16,17,18]) #存放正视图GroundTruth的文件夹路径
GroundTruth = np.loadtxt('D:\\bishe\\3DZeF20\\3DZeF20\\train\\ZebraFish-04\\gt\\gt.txt', delimiter=',' , usecols=[0,1,7,8,9,10,11]) #存放顶视图GroundTruth的文件夹路径

print(GroundTruth.shape)
print(GroundTruth.shape[0])

# WSI_MASK_PATH = 'D:\\bishe\\3DZeF20\\3DZeF20\\train\\ZebraFish-04\\imgF' #存放正视图图片的文件夹路径  
WSI_MASK_PATH = 'D:\\bishe\\3DZeF20\\3DZeF20\\train\\ZebraFish-04\\imgT' #存放正视图图片的文件夹路径
...
for path in paths:
    ...
    while((GroundRow[0]==CountImage)&(CountText<GroundTruth.shape[0])):
        ...
        # if(GroundRow[6]):
        #     dst = img[int(GroundRow[3]):int(GroundRow[3]+GroundRow[5]),int(GroundRow[2]):int(GroundRow[2]+GroundRow[4])]
        #     cv2.imshow("        "+str(CountText),dst)
