...
ap.add_argument('-of', '--outputFolder', help='path to the output folder')
...
if args.get('outputFolder', None) is None:
    print('Please specify the path to the output folder')
    sys.exit()
else:
...
result={}
result["Distortion"]=cam.dist.tolist()
result["K"]=cam.K.tolist()
outputName = os.path.join(outputFolder, 'cam_intrinsic.json')
